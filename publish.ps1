...
$versionsmaller = "${VERSION_MAJOR}.${VERSION_MINOR}"
$versionsmall = "${versionsmaller}.${VERSION_PATCH}"
...
MSBuild.exe AutoAway.sln /p:Configuration=Release /p:Platform="Win32" /p:VERSION_MAJOR=${VERSION_MAJOR} /p:VERSION_MINOR=${VERSION_MINOR} /p:VERSION_PATCH=${VERSION_PATCH} /p:VERSION_BUILD=${VERSION_BUILD}
MSBuild.exe AutoAway.sln /p:Configuration=Release /p:Platform="x64" /p:VERSION_MAJOR=${VERSION_MAJOR} /p:VERSION_MINOR=${VERSION_MINOR} /p:VERSION_PATCH=${VERSION_PATCH} /p:VERSION_BUILD=${VERSION_BUILD}

fReplace ".\Publish\data\package.ini" "__VERSION__" "${versionsmaller}"
fReplace ".\Publish\data\package.ini" "__PLATFORM__"  "${platform}"

$pluginpath = ".\Publish\AutoAway-${versionsmaller}.ts3_plugin"
...
.\Tools\7zip\x64\7za.exe -tzip a .\Publish\AutoAway-${versionsmaller}.ts3_plugin .\Publish\data\*